...
def main() -> int:
    ...
    with patch("app.main.run_migrations", lambda: None):
        ...
        try:
            ...
            bob_player_id = next(p["id"] for p in players if p.get("linked_user_id") == bob_user["id"])
            ...
            if bob_after["current_round"] < alice_state["current_round"]:
                fail("round", f"bob={bob_after['current_round']} alice={alice_state['current_round']}")
            ...
            if "Bob Friend" not in friend_names:
                fail("leaderboard_names", f"names={sorted(friend_names)} board={friends_board.json()}")
