...
def test_live_turn_flow(client):
    ...
    assert turn.json()["current_player"] == "P2"
    assert turn.json()["standings"][0]["total_score"] == 12
    ...
    assert challenge.json()["current_player"] == "P1"
    assert challenge.json()["current_round"] == 2
...
def test_submit_updates_standings_and_advances(client):
    ...
    body = first.json()
    assert body["current_player"] == "P2"
    assert body["standings"][0]["total_score"] == 12
    ...
    standings = second.json()["standings"]
    assert standings[0]["total_score"] == 12
    assert standings[1]["total_score"] == 20
